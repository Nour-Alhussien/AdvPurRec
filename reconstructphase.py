...
def reconstruction_module(model, data, lr=25, rec_iter=4, rand_initi=5, dim=204):
    r"""
    ...
    data = data[None, :].repeat(rand_initi, 1).to(device)

    # generate rand_init random noise
    z_hat = torch.randn(size=[rand_initi, dim]).to(device)
...
def reconstruction_pipeline(advdataset, diffusionModel, reciter=4, randiniti=5, mask=None, dim=204):
    bestReconstructions = torch.Tensor(size=[advdataset.shape[0], dim])
    z_ = torch.Tensor(size=[advdataset.shape[0], dim])
    ...
    for i in tqdm(range(advdataset.shape[0])):
        batch = advdataset[i]
        if mask is not None:
            batch = batch.masked_select(mask)
        x, recon, z_hat, recon_error[i] = reconstruction_module(
            diffusionModel, batch, rand_initi=randiniti, rec_iter=reciter, dim=dim
        )
